...
public class EntityPlayerSPTransformer extends UpsilonMiniTransformer {
	...
	public void patchOnLivingUpdate(PatchContext ctx) {
		if (UpsilonFixesConfig.sprintKey) {
			ctx.search(
				ALOAD(0),
				INVOKEVIRTUAL("net/minecraft/client/entity/EntityPlayerSP", "isSneaking", "()Z"),
				IFEQ(new LabelNode()),
				ALOAD(0),
				ICONST_0(),
				PUTFIELD("net/minecraft/client/entity/EntityPlayerSP", "sprintToggleTimer", "I")
			).jumpBefore();
			
			ctx.add(
				ALOAD(0),
				INVOKESTATIC(hooks(), "onLivingUpdate", "(Lnet/minecraft/client/entity/EntityPlayerSP;)V")
			);
		}
		...
		if (UpsilonFixesConfig.guisInPortals) {
			...
			ctx.search(
				ALOAD(0),
				GETFIELD("net/minecraft/client/entity/EntityPlayerSP", "mc", "Lnet/minecraft/client/Minecraft;"),
				ACONST_NULL(),
