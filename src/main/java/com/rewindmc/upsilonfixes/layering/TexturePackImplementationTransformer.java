...
@Patch.Class("net.minecraft.client.texturepacks.TexturePackImplementation")
...
public class TexturePackImplementationTransformer extends UpsilonMiniTransformer {
	...
	public void patchGetResourceAsStream(PatchContext ctx) {
		...
		ctx.add(
			...
			ALOAD(0),
			INSTANCEOF("net/minecraft/client/texturepacks/TexturePackDefault"),
			IFZ(Lnotdefault),
			ALOAD(0),
			ALOAD(1),
			...
			ALOAD(0),
			INVOKESTATIC("com/rewindmc/upsilonfixes/layering/Layering", "isLayerPack", "(Lnet/minecraft/client/texturepacks/ITexturePack;)Z"),
			IFZ(Lcontinue),
			ACONST_NULL(),
			ARETURN(),
			Lcontinue
